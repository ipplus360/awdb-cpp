...
{
	std::string msg = AWDB_strerror( code );
	if ( msg.empty() )
	{
		msg = "unknown AWDB error #" + std::to_string( code );
	}
...
std::error_code GeoLite2PP::make_error_code( GeoLite2PP::AWDBStatus s )
{
	return std::error_code( static_cast<int>(s), get_error_category() );
}


std::error_condition GeoLite2PP::make_error_condition( GeoLite2PP::AWDBStatus s )
{
