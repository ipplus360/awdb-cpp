 ...
 *
 * Note that GeoLite2++ builds on top of the existing C library "libaiwendb".  GeoLite2++ provides alternate C++
 * bindings, it is not a replacement!  GeoLite2++ was written to provide an easier method from C++ to access aiwen's
...
namespace GeoLite2PP
{
	...
	/** The DB class is the primary class used by GeoLite2++.  It is used to point to the aiwen's GeoLite2's database
	...
	class DB final
	{
		public:
			...
			/** Constructor.
			* @param [in] database_filename The database filename is the @p .awdb file typically downloaded from
			* aiwen.  Normally, this file is called @p GeoLite2-City.awdb and may be downloaded for free from
			*
			* working directory, or an absolute path and filename.
			*
			* ~~~~
			* std::string filename = "/opt/my_project_files/GeoLite2-City.awdb";
			*
			* GeoLite2PP::DB db( filename );
			*
			* std::cout << db.get_awdb_lib_version()  << std::endl;
			...
			* aiwen database must be available for GeoLite2++ to work.
			*/
			DB( const std::string &database_filename = "IP_city_2020W16_single_WGS84.awdb" );

			/** Get the aiwen library version number.
			 *
			 * ~~~~
			 * GeoLite2PP::DB db;
			 * std::cout << db.get_lib_version_awdb() << std::endl;
			 * ~~~~
			 *
			 * Example output:
			 * ~~~~{.txt}
			 ...
			 * ~~~~
			 */
			std::string get_lib_version_awdb( void ) const;
			...
			/** Get the database metadata.  This returns a raw @p AWDB_metadata_s structure.  This call is intended
			 * mostly for internal purposes, or to call directly into the original C AWDB API.
			 * @see @ref get_metadata()
			 */
			AWDB_metadata_s get_metadata_raw( void );
			...
			/** Look up an IP address.  This returns a raw @p AWDB_lookup_result_s structure.  This call is intended
			 * mostly for internal purposes, or to call directly into the original C AWDB API.
			 * @see @ref lookup()
			 */
			AWDB_lookup_result_s lookup_raw( const std::string &ip_address );
			 ...
			 * efficient than the other @p get_field() method which takes a @p AWDB_lookup_result_s parameter.
			 *
			 * ~~~~
			 * GeoLite2PP::DB db;
			 ...
			 * ~~~~
			 * GeoLite2PP::DB db;
			 * AWDB_lookup_result_s result = db.lookup_raw( "65.44.217.6" );
			 ...
			 * ~~~~
			 */
			std::string get_field( AWDB_lookup_result_s *lookup, const std::string &language, const VCStr &v );
			 ...
			 * @p AWDB_free_entry_data_list() on the node.
			 *
			 * This call is intended mostly for internal purposes.  @see @ref lookup()  @see @ref get_metadata()
			 */
			std::string to_json( AWDB_entry_data_list_s *node );

		private:

			/// Internal handle to the database.  @see @p AWDB_open()
			AWDB_s awdb;

			/// Traverse a list of nodes and build up a JSON string.  This method is not exposed.  It is for internal use only.
			void create_json_from_entry( std::stringstream &ss, size_t depth, uint32_t data_size, AWDB_entry_data_list_s * &next, const bool in_array = false );
			...
			void add_to_map( GeoLite2PP::MStr &m, AWDB_lookup_result_s *node, const std::string &name, const std::string &language, const VCStr &v );
	};
}
